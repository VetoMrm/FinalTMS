...
export class OpenTicketComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if(this.designation=='Admin'){
       this.getAdminOpenTickets(this.userId);
    }
    if(this.designation=='User'){
        this.getUserOpenTickets(this.userId);
    }
    if(this.designation=='Assigned_User'){
       this.getAssignedUserOpenTicket(this.userId);
    }
  ...
  getAdminOpenTickets(userId:number){
   this.ticketService.getAdminOpenTickets(userId)
   .subscribe(response=>{
  ...
  getUserOpenTickets(userId:number){
    this.ticketService.getUserOpenTickets(userId)
    .subscribe(response=>{
    this.dataSource = new MatTableDataSource(response.object.ticketList);
  ...
  getAssignedUserOpenTicket(userId:number){
    this.ticketService.getAssignedUserOpenTickets(userId)
    .subscribe(response=>{
    this.dataSource = new MatTableDataSource(response.object.ticketList);
