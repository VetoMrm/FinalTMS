...
export class ClosedTicketComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if(this.designation=='Admin'){
       this.getAdminClosedTickets(this.userId);
    }
    if(this.designation=='User'){
        this.getUserClosedTickets(this.userId);
    }
    if(this.designation=='Assigned_User'){
       this.getAssignedClosedTicket(this.userId);
    }
  ...
  getAdminClosedTickets(userId:number){
    this.ticketService.getAdminClosedTickets(userId)
    .subscribe(response=>{
  ...
  getUserClosedTickets(userId:number){
    this.ticketService.getUserClosedTickets(userId)
    .subscribe(response=>{
    this.dataSource = new MatTableDataSource(response.object.ticketList);
  ...
  getAssignedClosedTicket(userId:number){
    this.ticketService.getAssignedUserClosedTickets(userId)
    .subscribe(response=>{
    this.dataSource = new MatTableDataSource(response.object.ticketList);
